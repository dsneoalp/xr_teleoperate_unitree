...
teleop session is not joined. Run inside the robot-g1 container:

    python tests/portal_hw_encode_check.py
...
def main() -> int:
    ...
    if not os.environ.get("LIVEKIT_URL"):
        logger_mp.error("FAIL  LIVEKIT_URL missing; cannot publish frames")
        return 2
    ...
    try:
        ...
        if not track:
            logger_mp.error("FAIL  portal.yaml has no video tracks")
            return 1
        ...
        while time.time() - t0 < args.duration:
            ...
            if portal.hw_encode_confirmed:
                ...
                logger_mp.info(f"OK  HW encoder FD {fds} after {n} RGB frames")
                logger_mp.info(f"OK  FFI log {MMAPI_LOG!r}")
                return 0
            time.sleep(interval)
