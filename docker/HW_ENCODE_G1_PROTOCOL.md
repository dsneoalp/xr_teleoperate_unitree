...
Compose: `docker/compose.yml` + `docker/compose.g1.yml` service `robot-g1`  
Image: `xr-teleop:robot-g1` (`docker/Dockerfile.robot-g1`)  
Check script: `teleop/tests/portal_hw_encode_check.py` (WORKDIR in the image is `/app/teleop`)
...
Fail-closed encode is **CLI**, not Compose: pass `--require-hw-encode` to `teleop_robot.py`, or run the check script (always requires HW). Mock, operator, and PC `robot` stay software. `--no-img` never sends frames, so the post-frame gate does not run — do not use `--no-img` as an encode accept test.
...
- Prefer `./docker/run.sh robot-g1 python tests/portal_hw_encode_check.py` for encode accept. It does not start DDS and joins a **throwaway LiveKit room** (`$LIVEKIT_ROOM-hwcheck-<uuid>`), not the live teleop room.
- Keep the robot in a safe pose. ESTOP reachable. No walking/teleop unless the operator explicitly asks for T4.

**Do not**

- `docker compose -f docker/compose.yml -f docker/compose.g1.yml up robot-g1` / default CMD `python teleop_robot.py --ee dex3 --arm G1_29` while another stack owns the G1 (Enter Debug Mode / DDS). Without `--require-hw-encode` the process will not fail-closed on OpenH264.
...
| Repo | `git -C <repo> rev-parse --abbrev-ref HEAD` | the branch you were given |
...
**PASS:** image `xr-teleop:robot-g1` builds.  
...
```bash
./docker/run.sh robot-g1 bash -lc 'ls -l /dev/nvhost-msenc; echo NVIDIA_DRIVER_CAPABILITIES=$NVIDIA_DRIVER_CAPABILITIES; echo LD_LIBRARY_PATH=$LD_LIBRARY_PATH; test -f /opt/tegra-libs/libnvtvmr.so && echo nvtvmr=ok'
```
...
Compose does **not** set `SAG_REQUIRE_HW_ENCODE` or `--require-hw-encode`.
...
Optional: `--duration 15 --fps 30`. Confirm timeout follows `SAG_HW_ENCODE_CONFIRM_S` (default 15s). The check script always sets `require_hw_encode=True` and sets confirm timeout from `--duration`.
...
# CMD: python teleop_robot.py --ee dex3 --arm G1_29 --require-hw-encode
```
...
Try **once**, `robot-g1` service only, local compose override — not a committed default:
...
| Layer | Done when |
|---|---|
| Plumbing | nvidia runtime + msenc + Tegra + `video` + `lab-jetson` FFI in `xr-teleop:robot-g1` |
...
T2 env: NVIDIA_DRIVER_CAPABILITIES=  msenc in container: yes/no  --require-hw-encode used: yes/no
